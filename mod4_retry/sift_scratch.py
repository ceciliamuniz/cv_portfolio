...
#!/usr/bin/env python3
"""
Improved Image Stitching Implementation
Addresses the issues in the original code with better matching, RANSAC, and blending.
"""
...
def load_and_validate_images(img1_path, img2_path):
...
def detect_and_match_features(img1, img2):
    """Detect SIFT features and match them with ratio test"""
    ...
    # Better matching with BFMatcher and ratio test
    bf = cv2.BFMatcher()
...
def estimate_homography_ransac(kp1, kp2, matches):
    ...
    if len(matches) < 4:
        print("Error: Need at least 4 matches for homography estimation")
        return None, []
    ...
    # Count inliers
    inliers = np.sum(mask.ravel()) if mask is not None else 0
    print(f"RANSAC found {inliers} inliers out of {len(matches)} matches")
    
    return H, mask

def calculate_canvas_size(img1, img2, H):
    """Calculate the size of canvas needed for the panorama"""
    h1, w1 = img1.shape[:2]
    h2, w2 = img2.shape[:2]
    ...
    # Calculate translation to make all coordinates positive
...
def create_improved_panorama(img1, img2, H):
    """Create panorama with proper canvas size and blending"""
    canvas_width, canvas_height, translation = calculate_canvas_size(img1, img2, H)
    
    print(f"Canvas size: {canvas_width}x{canvas_height}")
    
    # Adjust homography with translation
    H_adjusted = translation @ H
    translation_img1 = translation
    
    # Warp both images to the canvas
    img1_warped = cv2.warpPerspective(img1, translation_img1, (canvas_width, canvas_height))
    img2_warped = cv2.warpPerspective(img2, H_adjusted, (canvas_width, canvas_height))
    
    # Create masks for blending
    mask1 = np.zeros((img1.shape[0], img1.shape[1]), dtype=np.uint8)
    mask1[:, :] = 255
    mask1_warped = cv2.warpPerspective(mask1, translation_img1, (canvas_width, canvas_height))
    
    mask2 = np.zeros((img2.shape[0], img2.shape[1]), dtype=np.uint8)
    mask2[:, :] = 255
    mask2_warped = cv2.warpPerspective(mask2, H_adjusted, (canvas_width, canvas_height))
    
    # Find overlap region
    overlap = cv2.bitwise_and(mask1_warped, mask2_warped)
    
    # Create blended result
    result = np.zeros((canvas_height, canvas_width, 3), dtype=np.uint8)
    
    # Non-overlap regions: use original images
    result[mask1_warped > 0] = img1_warped[mask1_warped > 0]
    result[mask2_warped > 0] = img2_warped[mask2_warped > 0]
    ...
    if np.any(overlap_indices):
        ...
        result[overlap_indices] = (
            img1_warped[overlap_indices].astype(np.float32) * 0.5 + 
            img2_warped[overlap_indices].astype(np.float32) * 0.5
        ).astype(np.uint8)
...
def main():
    ...
    # Load images
    img1, img2 = load_and_validate_images(img1_path, img2_path)
    ...
    if H is None:
        print("Error: Could not estimate homography")
        return
    
    # Create panorama
    print("Creating panorama...")
    panorama = create_improved_panorama(img1, img2, H)
    
    # Save and display result
    output_path = 'improved_panorama.jpg'
    cv2.imwrite(output_path, panorama)
    print(f"Panorama saved as {output_path}")
    
    # Display (optional - comment out for headless environments)
    cv2.imshow('Original Image 1', cv2.resize(img1, (400, 300)))
    cv2.imshow('Original Image 2', cv2.resize(img2, (400, 300)))
    cv2.imshow('Improved Panorama', cv2.resize(panorama, (800, 400)))
    cv2.waitKey(0)
